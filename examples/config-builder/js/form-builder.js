...
function createTranslationsInput(field, fullName, parentObj) {
  ...
  const addButton = createAddButton(() => {
    const language = createLanguage(container)
    container.insertBefore(language, addButton)
  })
  container.appendChild(addButton)

  function createLanguage(container) {
...
function createGroup(field, fullName, parentObj) {
  ...
  function addGroupInstance(beforeElement = null, index = 0) {
    ...
    if (field.repeatable) {
      ...
      const addButton = createAddButton(() => addGroupInstance(instanceContainer.nextSibling, index + 1))
      instanceContainer.appendChild(addButton)
  ...
  if (field.repeatable) {
    const addButton = createAddButton(() => addGroupInstance())
    container.appendChild(addButton)
  } else {
    addGroupInstance()
  }

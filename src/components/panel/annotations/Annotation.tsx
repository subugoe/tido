...
const Annotation: FC<Props> = React.memo(({ data, top, onToggle, isNested = false }) => {
  ...
  const { matchedMaps, updateMatchedMap } = useAnnotations()
  ...
  const matchedMap = matchedMaps[data.id] ?? {}
  ...
  function handleClick(e: React.MouseEvent<HTMLDivElement>) {
    ...
    const flippedMatchedAnnotationsMap = getFlippedNestedMatchedAnnotationsMap(matchedMap)
    ...
    <div className="px-3 pb-2">
      ...
      <div ref={annotationBodyRef} className={`transition-[height] duration-400 ease-in-out ${isLong && !isExpanded ? 'h-18 overflow-y-hidden' : 'h-fit'}`}  >
        ...
        { type !== 'Variant' && <GenericTextRenderer
          ...
          ignoreFilters={true}
          onSelect={onSelect}
          onUpdateMatchedAnnotationsMap={(map) => updateMatchedMap(data.id, map)}
        /> }
      </div>

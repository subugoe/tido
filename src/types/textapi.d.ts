declare global {
  ...
  interface Content {
    /**
     * Contenttype
     * @description A MIME type. If several Content Objects with the same MIME type are provided, these SHOULD be distinguished with a MIME type parameter where the key is type and the value can be freely chosen, e.g. text/htmltype=transcription.
     */
    contentType: string
    /**
     * Id
  ...
  interface Image {
    ...
    /**
     * Mimetype
     * @description The expected mimetype of the resource at `id`. Most likely either starting with `image/` or, in case of IIIF, `application/ld+jsonprofile="http://iiif.io/api/image/3/context.json`".
     */
    mimetype: ('application/ld+jsonprofile="http://iiif.io/api/image/3/context.json' | 'application/ld+jsonprofile="http://iiif.io/api/presentation/3/context.json') | string | null
  }
  /**
   * Integrity
  ...
  interface Item {
     ...
     */
    annotationCollection?: string | null
    /**
     * Contents
   ...
   * @enum {string}
   */
  type TextItemType = 'fragment' | 'section' | 'page' | 'full'
  /** ValidationError */

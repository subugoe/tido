...
import { useAnnotationsStore } from '@/stores/annotations';
 ...
 const initCollection = async (url: string) => {
    ...
    if (Array.isArray(collection.sequence) && collection.sequence.length > 0) {
      ...
      if (item) contentStore.initItem(itemUrl)      
    }
  };
...
return { activeContentUrl, collection, item, itemUrl, manifests, manifest,     // states
        collectionTitle, itemIndex,    // computed
        setCollection, setItem, setItemUrl, setManifests, setManifest, setActiveContentUrl,   //mutations as functions
        findActiveManifestIndex, findManifestIndexOfManifestInConfig, getManifest, isManifestPartInsideRangeValue,  // actions as functions
        isItemPartInsideRangeValue, initCollection, initManifest, initItem, getSupport
        }
})